...
export function setup() {
  ...
  for (let i = 1; i <= MAX_VUS; i++) {
    const email = `k6_mixed_${i}_${Date.now()}@test.com`;
    const password = 'K6Test@Pass123!';

    http.post(
      `${BASE}/api/v1/auth/register`,
      JSON.stringify({ email, username: `k6m${i}x${Date.now()}`, password }),
